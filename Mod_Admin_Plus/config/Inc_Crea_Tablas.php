<?php
  ...
  UNIQUE KEY `Usuario` (`Usuario`)
	...
	if(mysqli_query($db , $admin)){
			...
			//echo "LA FILE EXISTE <br>";
				}

	/************* CREAMOS LA TABLA FEEDBACK ****************/

	global $table_name_f; 		$table_name_f = "`".$_SESSION['clave']."feedback`";
  ...
  PRIMARY KEY  (`idv`)
	...
	(69, 0, 0, 0, 0)";
		if(mysqli_query($db, $vd)){
				global $table4; 	$table4 = "\t* OK INIT VALUES EN VISITAS ADMIN.".PHP_EOL;
		} else { 
				global $table4; 	
				$table4 = "\t* NO OK INIT VALUES EN ".$table_name_c." ".mysqli_error($db).PHP_EOL;
					}

	} else {global $table3; 	$table3 = "\t* NO OK TABLA VISITAS ADMIN. ".mysqli_error($db).PHP_EOL;
			global $table4; 	
			$table4 = "\t* NO OK INIT VALUES EN ".$table_name_c." ".mysqli_error($db).PHP_EOL;
			}
